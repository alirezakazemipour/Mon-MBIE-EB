...
class MonQCritic(Critic):
        ...
        for se in range(self.n_obs_env):
            for ae in range(self.n_act_env):
                if np.sum(c_joint_bar[se, :, ae, :]) < 0.01:
                    ...
                    continue
                if self._no_env[se, ae] == 0:
                    self._q_joint[se, :, ae, :] = 2 / (1 - self.gamma)
                    continue
                w = math.sqrt(math.log(self._no_env[se].sum(-1))) * self.A / math.sqrt(self._no_env[se, ae])
                for sm in range(self.n_obs_mon):
                    for am in range(self.n_act_mon):
                        s = se, sm
                        a = ae, am
                        if self._n_joint[*s, *a] == 0:
                            self._q_joint[*s, *a] = 2 / (1 - self.gamma)
                            continue
                        else:
    ...
    def reset(self):
        ...
        self._q_joint = np.ones(
            (self.n_obs_env, self.n_obs_mon, self.n_act_env, self.n_act_mon)) * 2 / (1 - self.gamma)
...
class MonQTableCritic(MonQCritic):
    ...
    def __init__(self,
        ...
        self._q_joint = np.ones((n_obs_env, n_obs_mon, n_act_env, n_act_mon)) * 2 / (1 - self.gamma)
